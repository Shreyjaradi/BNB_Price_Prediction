...
```
Converting "last_review" column to datetime column
```{r}
bnb_df[['last_review']] = strptime(bnb_df[['last_review']],format = "%Y-%m-%d")
```

Need to convert the categorical data to level 
```{r}
bnb_df[, c('neighbourhood_group', 'room_type', 'last_review')] = sapply(bnb_df[, c('neighbourhood_group', 'room_type', 'last_review')], unclass)
```
