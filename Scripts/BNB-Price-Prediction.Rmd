...
```{r}
predicted_val = predict(bnb_model_two, newdata = bnb_df_test)

bnb_lr_RMSE_regression = sqrt(mean( (bnb_df_test$price - predicted_val)**2 ))
bnb_lr_SSE = sum((bnb_df_test$price - predicted_val)**2)
bnb_lr_SSR = sum((predicted_val - mean(bnb_df_test$price)) ** 2)
bnb_lr_R2 = 1 - bnb_lr_SSE/(bnb_lr_SSE + bnb_lr_SSR)
bnb_lr_RMSE_regression
bnb_lr_SSE
bnb_lr_SSR
bnb_lr_R2
```
